...
- NEVER modify files under `target/` or `build/` directories. These are generated by the
  build system and annotation processors. If a generated file has errors, fix the SOURCE
  file that triggers the code generation (e.g., fix annotations, imports, or the pom.xml).
- When the same error pattern appears in multiple files, fix ALL of them in one pass
  rather than fixing one file at a time.
